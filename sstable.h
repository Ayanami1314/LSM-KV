...
class sstable_type {
  ...
  private:
    ...
    u64 binary_search(TKey key, u64 total, bool &exist,
                      bool use_BF = true) const;

  public:
    static u64 getCurID() { return ss_total_uid; }
    static void setCurID(u64 num) { ss_total_uid = num; }
    // static
    static void resetID();
    ...
    [[nodiscard]] static std::string get_filename(const Header &h) {
        return std::to_string(h.getTimeStamp()) + "_" +
               std::to_string(h.getMinKey()) + "~" +
               std::to_string(h.getMaxKey()) + ".sst";
    }
    ...
    static u64 cal_size(size_t number_of_kv,
                        u64 BF_size = config::bf_default_size);
